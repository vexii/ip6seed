...
describe("IPv6 Conversion Functions", () => {
  describe("ip6ToBigInt and bigIntToIp6", () => {
    it("should convert IPv6 to BigInt and back correctly", () => {
      ...
      for (const ipv6 of testCases) {
        const bigInt = ip6ToBigInt(ipv6);
        const backToIp6 = bigIntToIp6(bigInt);

        // Check that they represent the same address
        const originalBigInt = ip6ToBigInt(ipv6);
        const resultBigInt = ip6ToBigInt(backToIp6);
        expect(originalBigInt).toBe(resultBigInt);
      }
    });

    it("should handle compressed IPv6 notation", () => {
      const compressed = "2001:db8::1";
      const bigInt = ip6ToBigInt(compressed);
      const backToIp6 = bigIntToIp6(bigInt);
      // The function expands compressed notation, so we expect the expanded form
      expect(backToIp6).toBe("2001:db8:0:0:0:0:0:1");
    });

    it("should throw error for invalid IPv6", () => {
      const invalidCases = [
        "invalid",
        "fe80::2fb4:5866:4c4d:b951:extra",
        "fe80::2fb4:5866:4c4d:b951:extra:segment"
      ];

      for (const invalid of invalidCases) {
        expect(() => ip6ToBigInt(invalid)).toThrow();
      }
    });
  });
  ...
  describe("ip6ToSentence", () => {
    it("should convert IPv6 to sentence", () => {
      const ipv6 = "fe80::2fb4:5866:4c4d:b951";
      const sentence = ip6ToSentence(ipv6);

      expect(typeof sentence).toBe("string");
      expect(sentence.length).toBeGreaterThan(0);
      expect(sentence.split(" ").length).toBeGreaterThan(10);
    });

    it("should generate consistent sentences for same IPv6", () => {
      const ipv6 = "fe80::2fb4:5866:4c4d:b951";
      const sentence1 = ip6ToSentence(ipv6);
      const sentence2 = ip6ToSentence(ipv6);
  ...
  describe("extractWords", () => {
    it("should extract words from sentence", () => {
      ...
      expect(words.every((word: string) => typeof word === "string")).toBe(true);
      expect(words.every((word: string) => wordlist.includes(word))).toBe(true);
    });
  });
  ...
  describe("sentenceToIp6", () => {
    it("should convert sentence back to IPv6", () => {
      const originalIpv6 = "fe80::2fb4:5866:4c4d:b951";
      const sentence = ip6ToSentence(originalIpv6);
      const recoveredIpv6 = sentenceToIp6(sentence);

      // Check that they represent the same address
      const originalBigInt = ip6ToBigInt(originalIpv6);
    ...
    it("should throw for invalid sentences", () => {
      const invalidSentences = [
        "Invalid sentence",
        "Too short sentence"
      ];

      for (const sentence of invalidSentences) {
        expect(() => sentenceToIp6(sentence)).toThrow();
      }
    });
  });
  ...
  describe("Integration Tests", () => {
    it("should handle complete round-trip conversion", () => {
      ...
      const sentence2 = ip6ToSentence(recoveredIpv6);
      expect(sentence).toBe(sentence2);
    });
